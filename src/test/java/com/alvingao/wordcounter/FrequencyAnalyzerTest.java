...
public class FrequencyAnalyzerTest {
    ...
    @Test
    public void testGetSentencesWithWordSingleton() throws TokenizationException {
        List<Sentence> corpus = (new Tokenizer()).corpusToSentences(this.mobyDick);
        FrequencyAnalyzer classUnderTest = new FrequencyAnalyzer(corpus);

        List<Sentence> actual = classUnderTest.getSentencesWithWord("surprising");
        List<Sentence> expected = new ArrayList<>(Arrays.asList(new Sentence(new Word("There"), new Word("is"),
                new Word("nothing"), new Word("surprising"), new Word("in"), new Word("this"), new Punctuation("."))));
